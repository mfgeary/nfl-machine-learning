...
In the recipe, data that should remain in the data set but are not predictors are assigned to appropriate roles. All data numeric data is normalized and highly correlated predictors are removed. I intentionally did this step before creating dummy variables, since I want the nominal predictors to stay in the data. I also chose not to impute data because different positions have different statistics, so the missing values should be missing or 0 to indicate that player's performance in stats not relating to their position.
...
In this summary, it is clear that player position has a significant impact on fantasy points scored, with the highest scoring players on the field scoring the most in fantasy points. Quarterbacks consistently score significantly more points than other positions.
...
This chart shows the distribution of fantasy points by position. Notice that `QB`, `RB`, `TE`, and `WR` score the most fantasy points. This is linked to the fact that most offensive players score highest in fantasy football, especially players who score more touchdowns and gain first downs. `QB` has the highest mean in the distribution, but `RB` has the highest overall score. This indicates that consistently, quarterbacks are the highest scorers in fantasy football, but running backs and wide recievers can have high-scoring individual games when they score lots of points on the field.
...
all_data %>%
  ...
  scale_color_manual(values = c("QB" = 'red',
                                ...
                                "RB" = 'yellow',
                                'TE' = 'green')) +
  labs(x = "Average Fantasy Points", y = "Weighted Opportunity Rating", title = "Average Fantasy Points vs Weighted Opportunity Rating")
```
...
all_data %>%
  group_by(gsis_id) %>%
  summarise(avg = mean(fantasy_points), gsis_id = gsis_id, full_name = full_name, headshot_url = headshot_url, position = position) %>%
  distinct() %>%
  filter(avg >= 20) %>%
  ggplot() + 
  geom_col(aes(x = gsis_id, y = avg, fill = position)) +
  theme_minimal() + 
