...
all_data <- all_data %>% filter(!is.na(fantasy_points_ppr)) %>% mutate_if(is.character, as.factor)
```

```{r}
recipe <- all_data %>% recipe(fantasy_points ~ .) %>%
  update_role(gsis_id, game_id, new_role = "id") %>%
...
prep(recipe, all_data)

all_data %>% select(gsis_id, full_name) %>% distinct
```
