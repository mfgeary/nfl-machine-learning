...
svm_tuning_grid <- grid_max_entropy(parameters(svm_rbf_model), size = 100)
...
nn_tuning_grid <- grid_max_entropy(parameters(nn_model), size = 100)
...
rf_tuning_grid <- grid_max_entropy(rf_param, size = 100)
...
xgb_final_test <- augment(xgb_cv_fit, new_data = nfl_test)

svm_final_fit <- svm_tune_wkflow %>% fit(data = nfl_train)
svm_final_test <- augment(svm_cv_fit, new_data = nfl_test)

nn_final_fit <- nn_tune_wkflow %>% fit(data = nfl_train)
nn_final_test <- augment(nn_cv_fit, new_data = nfl_test)

rf_final_fit <- rf_tune_wkflow %>% fit(data = nfl_train)
rf_final_test <- augment(rf_cv_fit, new_data = nfl_test)
```
