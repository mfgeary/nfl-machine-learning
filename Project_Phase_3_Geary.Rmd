...
player_summary %>% kable(
    ...
    col.names = c("Position", "Count", "Avg Fantasy Points", "Standard Deviation", "Scoring Players")
  )
...
all_data %>% 
  ...
  group_by(gsis_id) %>%
  summarise(mean = mean(fantasy_points), position = position) %>%
  ggplot(aes(x = mean)) + 
  ...
  labs(
      x = 'Fantasy Points',
      y = 'Count',
      title = 'Distribution of Average Fantasy Points by Position',
      caption = 'The distribution of average fantasy points scored by each player in each NFL position.',
      tag = 'Figure 3'
  )
```

Fantasy football points are scored as a function of real statistics from performance in NFL games.
