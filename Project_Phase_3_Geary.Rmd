...
I used a radial support vector machine because these tend to perform very highly on a wide variety of types of data sets. I chose a radial kernel because I knew that my data was not close to linear or polynomial. I also used a neural net because it is a similarly high performing model. I used the random forest model as well because I suspected that decisions tree would perform highly with my data. Finally, I used an elastic net model to provide a comparison using a type of linear regression.
...
From `Figure 3`, we see that the model tends to underpredict on outlier games were players score very highly. However, the model tends to slightly overpredict on games were player have very low fantasy scores. Generally, the model is pretty balanced in its predictions.
...
The results of this study limited are due to limited computational power. Ideally, I would use a better method such as k-nearest neighbors or a decision tree for imputing missing data. I think with better imputation, the radial support vector machine and the neural network models would perform comparably with the Stochastic Gradient Boosting. I also would like to use larger tuning grids for selecting the parameters for each model. These were limited due to computational time in this study. 

For future research, a model would work well if it was tuned with all NFL data since the creation of Fantasy Football. This data is readily accessible, and it would help create the best model for predicting future fantasy football scores. One future area of research is predicting the performance of rookies before the NFL season even begins. A different data set with collegiate data would be required to predict the fantasy points of rookies before they step foot in an NFL game.
